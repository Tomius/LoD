#ifndef ENGINE_MESH_ANIMATED_MESH_RENDERER_SKINNING_INL_HPP_
#define ENGINE_MESH_ANIMATED_MESH_RENDERER_SKINNING_INL_HPP_
    ...
    for(size_t i = 0; i < pMesh->mNumBones; i++) {
      ...
      if(skinning_data_.bone_mapping.find(bone_name)
        ...
        skinning_data_.bone_info[bone_index].bone_offset =
          engine::convertMatrix(pMesh->mBones[i]->mOffsetMatrix);
...
inline void AnimatedMeshRenderer::setupBones(oglwrap::LazyVertexAttribArray boneIDs,
  ...
  if(skinning_data_.is_setup_bones) {
    throw std::logic_error(
      "AnimatedMeshRenderer::setupBones is called multiply times on the same object"
    );
  } else {
...
#endif // ENGINE_MESH_ANIMATED_MESH_RENDERER_SKINNING_INL_HPP_
