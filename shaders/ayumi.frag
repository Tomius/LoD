...
const float specular_shininess = 60.0f;
...
void main() {
    ...
    vec3 color = texture(tex, vert.texCoord).rgb;

    fragColor = color * AmbientColor() * (SunPower() * (diffuse_power + specular_power) + AmbientPower());
}
